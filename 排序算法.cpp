...
将最后一个数据的值赋值给根节点，然后再从分界点开始进行一次从上到下的调整，  


调整时先在左右儿子结点中找最小的，  


如果父节点比这个最小的子节点还小说明不需要调整了，反之将父节点和它交换后再考虑后面的结点。
```
// 从i节点开始调整，n为节点总数，从0开始计算第i节点的子节点为2*i +1, 2*i +2
...
//使用最小堆排序之后是递减数组，要得到递增数组，可以使用最大堆
由于每次重新恢复堆的时间复杂度为O(logN)，共N - 1次重新恢复堆操作，
再加上前面建立堆时N / 2次向下调整，每次调整时间复杂度也为O(logN)。
